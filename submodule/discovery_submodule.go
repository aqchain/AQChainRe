...
func NewDiscoverySubmodule(ctx context.Context, bsConfig *config.BootstrapConfig, network *NetworkSubmodule) (DiscoverySubmodule, error) {
	...
	return DiscoverySubmodule{
		...
		HelloHandler:   discovery.NewHelloProtocolHandler(network.Host, network.NetworkName),
	}, nil
}
...
func (m *DiscoverySubmodule) Start() error {
	// Start bootstrapper.
	...
	peerDiscoveredCallback := func(ci *types.Block,id peer.ID) {
		m.PeerTracker.Track(ci,id)
	...
	// chain head callback
	chainHeadCallback := func() (types.Block, error) {

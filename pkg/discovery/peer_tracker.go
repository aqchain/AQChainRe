...
func NewPeerTracker(self peer.ID, trust ...peer.ID) *PeerTracker {
	...
	return &PeerTracker{
		peers:   make(map[peer.ID]*types.Block),
		trusted: trustedSet,
		self:    self,
	}
}
...
func (tracker *PeerTracker) SelectHead() (*types.Block, error) {
	heads := tracker.listTrusted()
	if len(heads) == 0 {
		return nil, errors.New("no peers tracked")
	}
	sort.Slice(heads, func(i, j int) bool {
		return heads[i].Number().Cmp(heads[j].Number()) > 0
	})
	return heads[0], nil
}

// Track adds information about a given peer.ID
func (tracker *PeerTracker) Track(ci *types.Block,id peer.ID) {
	tracker.mu.Lock()
	defer tracker.mu.Unlock()

	_, tracking := tracker.peers[id]
	_, trusted := tracker.trusted[id]
	tracker.peers[id] = ci
...
func (tracker *PeerTracker) List() []*types.Block {
	tracker.mu.Lock()
	defer tracker.mu.Unlock()

	var tracked []*types.Block
	for _, ci := range tracker.peers {
		tracked = append(tracked, ci)
	}
	out := make([]*types.Block, len(tracked))
	copy(out, tracked)
	return out
}
...
func (tracker *PeerTracker) listTrusted() []*types.Block {
	tracker.mu.Lock()
	defer tracker.mu.Unlock()

		...
		case <-update:
			...
			for pid, fails := range expire() {
				if peer := d.peers.Peer(pid); peer != nil {
					...
					if fails > 2 {
						peer.log.Trace("ConfirmationData delivery timed out", "type", kind)
						setIdle(peer, 0)
					} else {
			...
			if pending() == 0 {
				if !inFlight() && finished {
					log.Debug("ConfirmationData fetching completed", "type", kind)
					return nil
				}
				break
			}
...
func (d *Downloader) commitPivotBlock(result *fetchResult) error {
	...
	if err := d.syncPocContextState(b.Header().PocContext); err != nil {
		return err
	}
...
// Todo: sync poc context in concurrent
func (d *Downloader) syncPocContextState(context *types.PocContextProto) error {
	roots := []common.Hash{
		context.CandidateHash,
		context.ContributionHash,
		context.LatestTxHash,
		context.EpochHash,
		context.MintCntHash,
	}

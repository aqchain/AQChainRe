...
func mockNewPocContext(db ethdb.Database) *types.PocContext {
	pocContext, err := types.NewPocContextFromProto(db, &types.PocContextProto{})
	if err != nil {
		return nil
	}
	candidate := []byte{}
	...
	pocContext.SetValidators(addresses)
	for j := 0; j < len(MockEpoch); j++ {
		candidate = common.HexToAddress(MockEpoch[j]).Bytes()
		pocContext.CandidateTrie().TryUpdate(candidate, candidate)
	}
	return pocContext
}
...
func TestUpdateMintCnt(t *testing.T) {
	...
	pocContext := mockNewPocContext(db)
	...
	beforeUpdateCnt := getMintCnt(blockTime/epochInterval, miner, pocContext.MintCntTrie())
	updateMintCnt(lastTime, blockTime, miner, pocContext)
	afterUpdateCnt := getMintCnt(blockTime/epochInterval, miner, pocContext.MintCntTrie())
	...
	setMintCntTrie(blockTime/epochInterval, miner, pocContext.MintCntTrie(), int64(1))

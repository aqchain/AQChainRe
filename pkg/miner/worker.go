...
func (self *worker) mintBlock(now int64) {
	engine, ok := self.engine.(*poc.Poc)
	if !ok {
		log.Error("Only the poc engine was allowed")
		return
	}
	err := engine.CheckValidator(self.chain.CurrentBlock(), now)
	if err != nil {
		switch err {
		case poc.ErrWaitForPrevBlock,
			poc.ErrMintFutureBlock,
			poc.ErrInvalidBlockValidator,
			poc.ErrInvalidMintBlockTime:
...
func (self *worker) makeCurrent(parent *types.Block, header *types.Header) error {
	...
	pocContext, err := types.NewPocContextFromProto(self.chainDb, parent.Header().PocContext)
	if err != nil {
		return err
	}
	work := &Work{
...
func (env *Work) commitTransaction(tx *types.Transaction, bc *core.BlockChain, coinbase common.Address) (error, []*types.Log) {
	...
	if err != nil {
		...
		env.pocContext.RevertToSnapShot(pocSnap)
		return err, nil
	}

...
var (
	...
	// ConfirmationData item prefixes (use single byte to avoid mixing data types, avoid `i`).
...
func GetBlock(db DatabaseReader, hash common.Hash, number uint64) *types.Block {
	...
	// add pocContext to block
	block.PocContext = getPocContextTrie(db.(ethdb.Database), header)
	return block
}

func getPocContextTrie(db ethdb.Database, header *types.Header) *types.PocContext {
	pocContextProto := header.PocContext
	if pocContextProto != nil {
		pocContext, err := types.NewPocContextFromProto(db, pocContextProto)
		if err != nil {
			return nil
		}
		return pocContext

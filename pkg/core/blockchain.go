...
// Processor.
func NewBlockChain(chainDb ethdb.Database, config *params.ChainConfig, engine consensus.Engine) (*BlockChain, error) {
...
// StateRecordAt returns a new mutable state based on a particular point in time.
func (bc *BlockChain) StateRecordAt(root common.Hash) (*state.StateDBRecord, error) {
	return state.NewRecord(root, bc.stateCache)
}
...
func (bc *BlockChain) WriteBlockAndState(block *types.Block, receipts []*types.Receipt, state *state.StateDB, stateRecord *state.StateDBRecord) (status WriteStatus, err error) {
	bc.wg.Add(1)
	defer bc.wg.Done()
	...
	if _, err := block.PocContext.CommitTo(batch); err != nil {
		return NonStatTy, err
	}
	...
	if reorg {
		...
		//
		if err := WritePreimages(bc.chainDb, block.NumberU64(), stateRecord.Preimages()); err != nil {
			return NonStatTy, err
		}
...
func (bc *BlockChain) insertChain(chain types.Blocks) (int, []interface{}, []*types.Log, error) {
	...
	for i, block := range chain {
		...
		block.PocContext, err = types.NewPocContextFromProto(bc.chainDb, parent.Header().PocContext)
		...
		err = bc.Validator().ValidatePocState(block)
		if err != nil {
			bc.reportBlock(block, receipts, err)
			return i, events, coalescedLogs, err
		}
		// Validate validator
		pocEngine, isPoc := bc.engine.(*poc.Poc)
		if isPoc {
			err = pocEngine.VerifySeal(bc, block.Header())
			if err != nil {
				bc.reportBlock(block, receipts, err)
				return i, events, coalescedLogs, err
			}

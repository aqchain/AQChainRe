...
func (v *BlockValidator) ValidatePocState(block *types.Block) error {
	header := block.Header()
	//fmt.Println(header.String())
	//fmt.Println(block.String())
	localRoot := block.PocCtx().Root()
	remoteRoot := header.PocContext.Root()
	if remoteRoot != localRoot {
		return fmt.Errorf("invalid poc root (remote: %x local: %x)", remoteRoot, localRoot)
	}
	return nil
}

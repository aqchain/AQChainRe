...
type Receipt struct {
	...
	PostStateRecord   []byte   `json:"rootRecord"`
...
func NewReceipt(root []byte,rootRecord []byte, failed bool) *Receipt {
	r := &Receipt{PostState: common.CopyBytes(root),PostStateRecord: common.CopyBytes(rootRecord)}
	if failed {

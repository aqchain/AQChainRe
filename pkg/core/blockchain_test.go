...
func newTestBlockChain(fake bool) *BlockChain {
	...
	blockchain, err := NewBlockChain(db, gspec.Config, engine)
	if err != nil {
		panic(err)
	}
...
func (bproc) ValidatePocState(block *types.Block) error { return nil }
...
func testReorgBadHashes(t *testing.T, full bool) {
	...
	ncm, err := NewBlockChain(bc.chainDb, bc.config, ethash.NewFaker())
	if err != nil {
...
func TestFastVsFullChains(t *testing.T) {
	...
	blocks, receipts := GenerateChain(gspec.Config, genesis, gendb, 1024, func(i int, block *BlockGen) {
		...
		if i%3 == 2 {
			for j := 0; j < i%4+1; j++ {
				tx, err := types.SignTx(types.NewTransaction(types.Binary, block.TxNonce(address), common.Address{0x00}, big.NewInt(1000), big.NewInt(1000), nil, nil), signer, key)
				if err != nil {
					panic(err)
				}
				block.AddTx(tx)
	...
	archive, _ := NewBlockChain(archiveDb, gspec.Config, ethash.NewFaker())
	defer archive.Stop()
	...
	fast, _ := NewBlockChain(fastDb, gspec.Config, ethash.NewFaker())
	defer fast.Stop()
...
func TestLightVsFastVsFullChainHeads(t *testing.T) {
	...
	light, _ := NewBlockChain(lightDb, gspec.Config, ethash.NewFaker())
...
func TestChainTxReorgs(t *testing.T) {
	...
	postponed, _ := types.SignTx(types.NewTransaction(types.Binary, 0, addr1, big.NewInt(1000), big.NewInt(1000), nil, nil), signer, key1)
	swapped, _ := types.SignTx(types.NewTransaction(types.Binary, 1, addr1, big.NewInt(1000), big.NewInt(1000), nil, nil), signer, key1)
	...
	chain, _ := GenerateChain(gspec.Config, genesis, db, 3, func(i int, gen *BlockGen) {
		switch i {
		case 0:
			pastDrop, _ = types.SignTx(types.NewTransaction(types.Binary, gen.TxNonce(addr2), addr2, big.NewInt(1000), big.NewInt(1000), nil, nil), signer, key2)

			gen.AddTx(pastDrop)  // This transaction will be dropped in the fork from below the split point
			gen.AddTx(postponed) // This transaction will be postponed till block #3 in the fork

		case 2:
			freshDrop, _ = types.SignTx(types.NewTransaction(types.Binary, gen.TxNonce(addr2), addr2, big.NewInt(1000), big.NewInt(1000), nil, nil), signer, key2)
	...
	blockchain, _ := NewBlockChain(db, gspec.Config, ethash.NewFaker())
	...
	chain, _ = GenerateChain(gspec.Config, genesis, db, 5, func(i int, gen *BlockGen) {
		switch i {
		case 0:
			pastAdd, _ = types.SignTx(types.NewTransaction(types.Binary, gen.TxNonce(addr3), addr3, big.NewInt(1000), big.NewInt(1000), nil, nil), signer, key3)
		...
		case 2:
			...
			freshAdd, _ = types.SignTx(types.NewTransaction(types.Binary, gen.TxNonce(addr3), addr3, big.NewInt(1000), big.NewInt(1000), nil, nil), signer, key3)
			gen.AddTx(freshAdd) // This transaction will be added exactly at reorg time

		case 3:
			futureAdd, _ = types.SignTx(types.NewTransaction(types.Binary, gen.TxNonce(addr3), addr3, big.NewInt(1000), big.NewInt(1000), nil, nil), signer, key3)

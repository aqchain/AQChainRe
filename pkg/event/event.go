...
// registered to handle events of certain types. Any operation
	...
	if mux.stopped {
		...
		for _, t := range types {
			...
			if find(oldsubs, sub) != -1 {
				panic(fmt.Sprintf("event: duplicate types %s in Subscribe", rtyp))
			}
...
// Post sends an event to all receivers registered for the given types.
